...
public class ELM327Interface implements J1850Interface
{
	...
	public ELM327Interface(HarleyDroidService harleyDroidService, BluetoothDevice device) {
		this(harleyDroidService, device, BusProtocol.J1850);
	}
	...
	public void connect(HarleyData hd) {
		if (D) Log.d(TAG, "connect bus=" + mBusProtocol);
	...
	public void startSend(String type[], String ta[], String sa[],
		...
		if (mCanBus) {
			// Active diagnostics not supported on CAN in V1 — stay polling
			if (mPollThread == null)
				startPoll();
			return;
		}
	...
	private boolean parseLine(byte[] bytes) {
		if (mCanBus)
			return HarleyCan.parse(bytes, mHD);
		return J1850.parse(bytes, mHD);
	}
	...
	private class ConnectThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				if (mCanBus) {
					// ISO 15765-4 CAN (11 bit, 500 kbaud) — HDLAN baud rate
					mSock.chat("ATSP6", "OK", AT_TIMEOUT);
					// Raw CAN frames (no ISO-TP auto formatting)
					try {
						mSock.chat("ATCAF0", "OK", AT_TIMEOUT);
					} catch (Exception e) {
						if (D) Log.d(TAG, "ATCAF0 not supported, continuing");
					}
				} else {
